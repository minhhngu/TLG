...
def main():
    ...
    if re.search(searchFor, searchMe):
        print(f'Found a total of {searchMe.count(searchFor)} matches!')
        # using .count to determine number of matches
    else:
        print("No match!")
...
if __name__ == "__main__":
    ...
    main()
    while True:
        print("Do you want search again? yes or no")
        kill = input("> ")
        if kill == "no":
            break
        else:
...
# while loop to receive user input until "no" reply